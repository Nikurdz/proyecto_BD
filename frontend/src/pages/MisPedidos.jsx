...
import { Package, RefreshCw, ChevronRight, Clock, ShoppingBag, Truck, CheckCircle } from 'lucide-react';
...
export default function MisPedidos() {
  ...
  useEffect(() => {
    ...
    const fetchPedidos = async () => {
      try {
        const res = await fetch(`http://${window.location.hostname}:5000/api/mis-pedidos`, {
          headers: {
  ...
  const getStatusBadge = (estado) => {
    ...
    if (status === 'pendiente') {
      return { bg: 'bg-yellow-50', text: 'text-yellow-700', icon: <Clock className="w-3.5 h-3.5" /> };
    } else if (status === 'enviado') {
      return { bg: 'bg-blue-50', text: 'text-blue-700', icon: <Truck className="w-3.5 h-3.5" /> };
    } else if (status === 'entregado') {
      return { bg: 'bg-emerald-50', text: 'text-emerald-700', icon: <CheckCircle className="w-3.5 h-3.5" /> };
    }
    return { bg: 'bg-stone-50', text: 'text-stone-700', icon: <Clock className="w-3.5 h-3.5" /> };
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="flex items-center space-x-3 mb-8">
        ...
        <h1 className="text-3xl font-extrabold text-stone-850">Historial de Compras</h1>
      </div>
      ...
      {pedidos.length === 0 ? (
        <div className="bg-white border border-stone-200 rounded-3xl p-10 text-center py-16 shadow-sm">
          ...
          <p className="text-stone-700 font-bold text-lg mb-2">No tienes pedidos registrados</p>
          <p className="text-stone-450 text-sm max-w-sm mx-auto mb-6">
            Una vez que realices una compra de forma simulada, podrás ver el estado e historial aquí.
          </p>
          ...
          {pedidos.map((pedido) => (
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-4 border-b border-stone-100 gap-3">
                <div>
                  <p className="text-[10px] text-stone-400 font-bold uppercase tracking-wider">Número de Pedido</p>
                  <p className="text-sm font-extrabold text-stone-850">#{pedido.id}</p>
                </div>
                
                <div className="flex items-center space-x-6">
                  <div>
                    <p className="text-[10px] text-stone-400 font-bold uppercase tracking-wider">Total del Pedido</p>
                    <p className="text-sm font-extrabold text-emerald-700">${pedido.total.toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-stone-400 font-bold uppercase tracking-wider mb-0.5">Estado</p>
                    <span className={`inline-flex items-center space-x-1 ${getStatusBadge(pedido.estado).bg} ${getStatusBadge(pedido.estado).text} text-[10px] font-black uppercase tracking-wider py-1 px-2.5 rounded-lg shadow-sm`}>
                      {getStatusBadge(pedido.estado).icon}
                ...
                {pedido.items.map((item, index) => (
                  <div key={index} className="flex justify-between items-center text-sm">
                    <div className="flex items-center space-x-2">
                      <ChevronRight className="w-4 h-4 text-emerald-655 shrink-0" />
